...
import { REGIONAL_BRANDS, getBrandForRegion, generateBrandCSSVars, type RegionalBrand } from '../../lib/regional-branding';
...
export class ThemeService {
  ...
  setRegion(region: Region): void {
    ...
    console.log(`🎨 ThemeService: Switching from ${this.currentRegionSubject.value} to ${region} theme`);
  ...
  private applyTheme(region: Region): void {
    ...
    console.log(`🎨 ThemeService: Applied ${region} theme with ${Object.keys(cssVars).length} CSS variables`);
  }
   ...
   */
  getInlineStyles(styleMap: Partial<Record<keyof RegionalBrand['colors'], string>>): { [key: string]: string } {
