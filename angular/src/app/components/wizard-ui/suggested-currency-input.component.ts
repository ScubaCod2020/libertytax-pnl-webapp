...
@Component({
  selector: 'lt-suggested-currency-input',
  standalone: true,
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => SuggestedCurrencyInputComponent),
      multi: true,
    },
  ],
  template: `
...
export class SuggestedCurrencyInputComponent implements ControlValueAccessor {
  @Input() value?: number;
  ...
  get displayPlaceholder(): string {
    return (
      this.placeholder || (this.suggestedValue ? `${this.suggestedValue.toLocaleString()}` : '')
    );
  }
