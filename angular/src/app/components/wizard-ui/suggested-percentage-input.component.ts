...
@Component({
  selector: 'lt-suggested-percentage-input',
  standalone: true,
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => SuggestedPercentageInputComponent),
      multi: true,
    },
  ],
  template: `
    <div style="display: flex; align-items: center;">
      ...
      />
      <span style="margin-left: 0.25rem; font-weight: 500; color: #6b7280;"> % </span>
    </div>
  `,
})
export class SuggestedPercentageInputComponent implements ControlValueAccessor {
  @Input() value?: number;
  ...
  get displayPlaceholder(): string {
    return this.placeholder || (this.suggestedValue ? `${this.suggestedValue.toFixed(1)}` : '');
  }
