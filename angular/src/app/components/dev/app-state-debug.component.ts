...
@Component({
  ...
  template: `
    <div *ngIf="show" class="app-state-debug">
      ...
      <div class="debug-info">ready: {{ isReady }} | hydrating: {{ isHydrating }}</div>
      <div class="debug-info">last saved: {{ savedAt }}</div>

      <div class="debug-actions">
        <button class="debug-btn" (click)="onSaveNow()">Save now</button>

        <button class="debug-btn" (click)="onDumpStorage()">Dump</button>

        <button class="debug-btn" (click)="onCopyJSON()">Copy JSON</button>

        <button class="debug-btn" (click)="onClearStorage()">Clear key</button>

        <button class="debug-btn" (click)="onShowWizard()">Wizard</button>
      </div>
    </div>
  `,
  styles: [
