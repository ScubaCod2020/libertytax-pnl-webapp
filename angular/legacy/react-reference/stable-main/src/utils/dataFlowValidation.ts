  ...
  criticalMappings.forEach((mapping) => {
    ...
    if (wizardValue !== undefined && appValue !== undefined && wizardValue !== appValue) {
      result.warnings.push(
        `Value mismatch: ${mapping.wizard} (${wizardValue}) ≠ ${mapping.app} (${appValue})`
      );
    }
  });
...
export function validateRoundTripDataIntegrity(
  ...
  roundTripFields.forEach(({ field, critical }) => {
    ...
    if (originalValue !== undefined && finalValue === undefined) {
      ...
      result.warnings.push(
        `Data modified in round trip: ${field} ${originalValue} → ${finalValue}`
      );
    }
  });
 ...
 */
export function validateDataFlow(wizardData: any, appState: any, sessionData: any): void {
  ...
  const totalIssues =
    wizardToApp.issues.length + appToStorage.issues.length + roundTrip.issues.length;
  const totalWarnings =
    wizardToApp.warnings.length + appToStorage.warnings.length + roundTrip.warnings.length;

  if (totalIssues === 0 && totalWarnings === 0) {
    console.log(
      '%c🎯 DATA FLOW VALIDATION: ALL CHECKS PASSED!',
      'color: #059669; font-weight: bold; font-size: 14px;'
    );
  } else if (totalIssues === 0) {
    console.log(
      `%c⚠️  DATA FLOW VALIDATION: ${totalWarnings} warnings found`,
      'color: #f59e0b; font-weight: bold;'
    );
  } else {
    console.log(
      `%c🚨 DATA FLOW VALIDATION: ${totalIssues} issues, ${totalWarnings} warnings`,
      'color: #dc2626; font-weight: bold;'
    );
  }
}
