  ...
  template: `
    ...
    >
      ...
      >
        ...
        <div *ngIf="data?.primaryMetric?.change" style="font-size: 0.8em; font-weight: normal; opacity: 0.8;">
          {{ changePrefix }}{{ data?.primaryMetric?.change?.percentage }}% {{ data?.primaryMetric?.change?.period }}
        </div>
      </div>

      <div [style.color]="colors.text" [style.marginBottom]="(showComparison || showInsights) ? '0.75rem' : '0'">
        <div style="font-size: 0.9em; opacity: 0.8; margin-bottom: 0.25rem;">{{ data?.primaryMetric?.label }}</div>
        <div style="font-size: 1.4em; font-weight: bold;">
          {{ formatValue(data?.primaryMetric?.value) }}
        </div>
      </div>

      <div *ngIf="data?.secondaryMetrics?.length" style="display: grid; gap: 0.5rem; margin-bottom: 0.75rem; font-size: 0.85em;"
           [style.gridTemplateColumns]="size === 'small' ? '1fr' : 'repeat(auto-fit, minmax(150px, 1fr))'">
        <div *ngFor="let metric of data?.secondaryMetrics" [style.color]="colors.text">
          <div style="opacity: 0.7">{{ metric.label }}:</div>
          <div style="font-weight: bold">
            {{ formatValue(metric.value) }}<ng-container *ngIf="metric.unit"> {{ metric.unit }}</ng-container>
          </div>
        </div>
      </div>

      <div *ngIf="showComparison && data?.comparison"
      ...
      <div *ngIf="showInsights && data?.insights?.length" style="font-size: 0.8em;">
        <div *ngFor="let insight of data?.insights; let i = index" [style.color]="colors.text" [style.marginBottom]="(i < (data?.insights?.length ?? 0) - 1) ? '0.5rem' : '0'" style="opacity: 0.9;">
