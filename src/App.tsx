...
export default function App() {
  ...
  React.useEffect(() => {
    ...
    if (wizardState.wizardCompleted && !wizardState.showWizard) {
      ...
      if (savedAnswers) {
        ...
        // Also hydrate last-known core values for UI consistency if present
        try {
          const env = persistence.loadEnvelope();
          if (env?.last) {
            if (typeof env.last.avgNetFee === 'number') appState.setANF(env.last.avgNetFee);
            if (typeof env.last.taxPrepReturns === 'number')
              appState.setReturns(env.last.taxPrepReturns);
          }
        } catch {}
      } else {
  ...
  // Reflect current page in URL hash and document title for easy orientation
  React.useEffect(() => {
    const page = appState.showWizard ? 'Wizard' : 'Dashboard';
    try {
      if (typeof window !== 'undefined') {
        const targetHash = appState.showWizard ? '#wizard' : '#dashboard';
    ...
    try {
      document.title = `Liberty Tax • P&L Budget & Forecast — ${page}`;
    } catch {}
  }, [appState.showWizard]);
      ...
      >
        ...
        {appState.showWizard ? (
          ...
          <div
            ...
            style={{
              display: 'grid',
              gridTemplateColumns: 'minmax(300px, 350px) minmax(400px, 500px) minmax(600px, 1fr)',
              gap: '1.5rem',
              alignItems: 'start',
            }}
          >
            <ProjectedPerformancePanel
              ...
              handlesTaxRush={
                appState.region === 'CA' && persistence.loadWizardAnswers()?.handlesTaxRush
              }
            />
