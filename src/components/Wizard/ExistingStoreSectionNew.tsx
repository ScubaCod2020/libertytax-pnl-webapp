...
/>
      ...
      <FormSection title="Projected Performance" icon="📈" description="Forecast this season based on targets">
        {/* Returns & ANF */}
        ...
        <FormField label="Gross Tax Prep Fees" helpText="Auto: Returns × Avg Net Fee (clear to re-auto)">
          <CurrencyInput
            value={answers.projectedGrossFees ?? (answers.taxPrepReturns && answers.avgNetFee
        ...
        <FormField
          label="Customer Discounts"
          helpText="Enter $ or %, the other will auto-calc"
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
              <span style={{ fontWeight: 500, color: '#6b7280' }}>$</span>
              <input
                ...
                onChange={(e) => {
                  ...
                  const projGross = answers.projectedGrossFees ?? (answers.taxPrepReturns && answers.avgNetFee
                    ? answers.taxPrepReturns * answers.avgNetFee
                    : undefined)
                  if (amt && projGross) {
            ...
            value={
              answers.projectedTaxPrepIncome ?? (() => {
                const projGross = answers.projectedGrossFees ?? (answers.taxPrepReturns && answers.avgNetFee
                  ? answers.taxPrepReturns * answers.avgNetFee
                  : undefined)
                return projGross !== undefined ? projGross - (answers.discountsAmt ?? 0) : undefined
              })()
            }
            placeholder="Auto-calculated"
          ...
          <CurrencyInput
            value={
              answers.projectedExpenses ?? (() => {
                const tpi = answers.projectedTaxPrepIncome ?? (() => {
                  const projGross = answers.projectedGrossFees ?? (answers.taxPrepReturns && answers.avgNetFee
                    ? answers.taxPrepReturns * answers.avgNetFee
                    : undefined)
        ...
        <NetIncomeSummary
          label="Projected"
          gross={(answers.projectedGrossFees ?? (answers.taxPrepReturns && answers.avgNetFee
            ? answers.taxPrepReturns * answers.avgNetFee
            : 0))}
