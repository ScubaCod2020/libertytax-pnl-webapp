...
export const STORAGE_KEY = `lt_pnl_v5_session_v2_${APP_VERSION}`;
...
export function usePersistence() {
  ...
  const loadEnvelope = (): PersistEnvelopeV1 | undefined => {
    try {
      ...
      if (!raw) {
        // Try migrate from previous storage key
        const legacyKey = `lt_pnl_v5_session_v1_${APP_VERSION}`;
        const legacy = localStorage.getItem(legacyKey);
        if (legacy) {
          dbg('Migrating legacy storage to v2', { from: legacyKey, to: STORAGE_KEY });
          try {
      ...
      if (!validatePersistenceData(parsed)) {
        console.warn(
          '💾 PERSISTENCE WARNING - Corrupted data detected, clearing storage for safety'
        );
