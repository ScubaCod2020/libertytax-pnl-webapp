// react-app-reference/react-app-reference/src/lib/calcs.ts
function calc(inputs) {
  try {
    const handlesTaxRush = inputs.handlesTaxRush ?? true;
    ...
    if (typeof import.meta !== "undefined" && import.meta.env?.DEV) {
      console.log("\u{1F9EE} CALC DEBUG (Server):", {
    ...
    console.log("\u{1F9EE} CALC DEBUG (Browser):", {
      region: inputs.region,
      avgNetFee: inputs.avgNetFee,
      taxPrepReturns: inputs.taxPrepReturns,
      taxRushReturns: taxRush,
    ...
    const totalExpenses = inputs.calculatedTotalExpenses || salaries + empDeductions + rent + telephone + utilities + localAdv + insurance + postage + supplies + dues + bankFees + maintenance + travelEnt + royalties + advRoyalties + taxRushRoyalties + misc;
...
function statusForCPR(v, t, inputs) {
  if (inputs) {
    ...
    const otherIncome = inputs.otherIncome || 0;
    const totalRevenue = taxPrepIncome + taxRushIncome + otherIncome;
    const revenuePerReturn = totalRevenue > 0 && inputs.taxPrepReturns > 0 ? totalRevenue / inputs.taxPrepReturns : 0;
...
function statusForMargin(v, t) {
  if (v >= 25) return "green";
  if (v >= 15) return "yellow";
  return "red";
}
function statusForNetIncome(v, t) {
  if (v >= 0) return "green";
