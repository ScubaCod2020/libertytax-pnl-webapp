...
export function useCalculations(inputs: CalculationInputs): CalculationResults {
  ...
  const calcInputs: Inputs = useMemo(
    ...
    [
      ...
      inputs.taxRushReturns,
      inputs.discountsPct,
      inputs.otherIncome,
      inputs.calculatedTotalExpenses,
      inputs.salariesPct,
      inputs.empDeductionsPct,
      inputs.rentPct,
      inputs.telephoneAmt,
      inputs.utilitiesAmt,
      inputs.localAdvAmt,
      ...
      inputs.duesAmt,
      inputs.bankFeesAmt,
      inputs.maintenanceAmt,
      inputs.travelEntAmt,
      inputs.royaltiesPct,
      inputs.advRoyaltiesPct,
      inputs.taxRushRoyaltiesPct,
      inputs.miscPct,
      inputs.thresholds,
    ]
  );
  ...
  const cprStatus = useMemo(
    () => statusForCPR(results.costPerReturn, inputs.thresholds, calcInputs),
    [results.costPerReturn, inputs.thresholds, calcInputs]
  );
  const nimStatus = useMemo(
    () => statusForMargin(results.netMarginPct, inputs.thresholds),
    [results.netMarginPct, inputs.thresholds]
  );
  const niStatus = useMemo(
    () => statusForNetIncome(results.netIncome, inputs.thresholds),
    [results.netIncome, inputs.thresholds]
  );
