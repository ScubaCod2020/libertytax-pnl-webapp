...
export default function WizardInputs({
  ...
  useEffect(() => {
    ...
    if (
      ...
      nextProjectedReturns = Math.round(
        answers.taxPrepReturns * (1 + answers.expectedGrowthPct / 100)
      );
    } else if (
    ...
    if (
      nextProjectedReturns !== undefined &&
      nextProjectedReturns !== answers.projectedTaxPrepReturns
    ) {
  ...
  React.useEffect(() => {
    console.log('🧙‍♂️ WIZARD INPUTS DEBUG - Page loaded/updated:', {
      ...
      totalFieldsSet: Object.keys(answers).filter((key) => (answers as any)[key] !== undefined)
        .length,
    });
  }, [answers]);
  ...
  const renderDualExpenseInput = (field: ExpenseField) => {
    ...
    const fieldLabel =
      field.id === 'taxRushShortagesPct' && answers.handlesTaxRush
        ? 'Shortages (TaxRush)'
        : field.label;
    ...
    const getCalculationBase = () => {
      ...
      if (field.id === 'taxRushRoyaltiesPct' && answers.handlesTaxRush) {
        ...
        const currentTaxRushGrossFees =
          answers.taxRushGrossFees && answers.expectedGrowthPct !== undefined
      ...
      const currentAvgNetFee =
        answers.projectedAvgNetFee ||
        (answers.avgNetFee && answers.expectedGrowthPct !== undefined
          ? answers.avgNetFee * (1 + answers.expectedGrowthPct / 100)
          : answers.avgNetFee);
      const currentTaxPrepReturns =
        answers.projectedTaxPrepReturns ||
        (answers.taxPrepReturns && answers.expectedGrowthPct !== undefined
          ? answers.taxPrepReturns * (1 + answers.expectedGrowthPct / 100)
          : answers.taxPrepReturns);

      const grossTaxPrepFees =
        currentAvgNetFee && currentTaxPrepReturns ? currentAvgNetFee * currentTaxPrepReturns : 0;
    ...
    const dollarValue = isFixed
      ? percentageValue
      : Math.round((calculationBase * percentageValue) / 100);
    ...
    const isTaxRushField =
      field.id === 'taxRushRoyaltiesPct' && answers.region === 'CA' && answers.handlesTaxRush;
  ...
  const renderCategorySection = (category: ExpenseCategory) => {
    ...
    return (
      ...
      >
        ...
        <div>{categoryFields.map(renderDualExpenseInput)}</div>
      </div>
    );
  };
  ...
  return (
    <div data-wizard-step="inputs" style={{ paddingLeft: '1rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                ...
                >
                  {answers.handlesTaxRush
                    ? 'Yes, we handle TaxRush returns'
                    : "No, we don't handle TaxRush"}
                </span>
              </div>
            )}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                ...
                >
                  {answers.hasOtherIncome
                    ? 'Yes, we have other income sources'
                    : 'No, only tax preparation'}
                </span>
              </div>
            )}
          </div>
      ...
      <p className="small" style={{ marginBottom: '1.5rem' }}>
        Enter your expected income and expense values. All fields have reasonable defaults, but
        customizing them will give you more accurate P&L projections.
      </p>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>💰 Income Drivers</div>
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder={
                answers.taxPrepReturns !== undefined ? String(answers.taxPrepReturns) : ''
              }
              disabled
              title="Configured in Step 1"
              style={{
              ...
              >
                ...
                <input
                  ...
                  style={{
                    ...
                    padding: '0.5rem',
                    ...(answers.storeType === 'existing'
                      ? { backgroundColor: '#ffffff', borderColor: '#0ea5e9' }
                      : {}),
                  }}
                />
              </div>
            ...
            >
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateAnswers({ taxRushAvgNetFee: +e.target.value || undefined })
                  }
        ...
        {(() => {
          ...
          const derivedReturns = (() => {
            if (answers.projectedTaxPrepReturns !== undefined)
              return answers.projectedTaxPrepReturns;
            if (
              answers.storeType === 'existing' &&
              answers.expectedGrowthPct !== undefined &&
              answers.taxPrepReturns !== undefined
          ...
          const adjustedTaxPrepReturns =
            currentTaxRushReturns > 0
              ? (currentTaxPrepReturns || 0) - currentTaxRushReturns
              : currentTaxPrepReturns || 0;
          ...
          const grossTaxRushFees =
            answers.region === 'CA' && answers.handlesTaxRush && currentTaxRushReturns > 0
          ...
          const revenueVariance =
            strategicTarget > 0 ? ((totalRevenue - strategicTarget) / strategicTarget) * 100 : 0;
            ...
            >
              ...
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns:
                    answers.region === 'CA' && answers.handlesTaxRush ? '1fr 1fr' : '1fr',
                  gap: '1rem',
                  marginBottom: '0.5rem',
                }}
              >
                ...
                <div>
                  ...
                  <div style={{ color: statusColor, fontSize: '0.8rem' }}>
                    Gross Tax Prep Fees: <strong>${grossTaxPrepFees.toLocaleString()}</strong>
                  </div>
                  <div style={{ color: statusColor, fontSize: '0.8rem' }}>
                    Returns: {adjustedTaxPrepReturns.toLocaleString()} @ $
                    {(currentAvgNetFee || 0).toLocaleString()}
                  </div>
                  <div style={{ color: '#dc2626', fontSize: '0.8rem' }}>
                    Less Discounts ({discountPct}%):{' '}
                    <strong>-${Math.round(discountAmount).toLocaleString()}</strong>
                  </div>
                  <div style={{ fontWeight: 'bold', color: '#059669', fontSize: '0.85rem' }}>
                    Net Tax Prep Income:{' '}
                    <strong>${Math.round(taxPrepIncome).toLocaleString()}</strong>
                  </div>
                </div>
                ...
                {answers.region === 'CA' && answers.handlesTaxRush && (
                  <div style={{ paddingLeft: '0.5rem', borderLeft: '2px solid #0ea5e9' }}>
                    ...
                    <div style={{ color: '#0369a1', fontSize: '0.8rem' }}>
                      Gross TaxRush Fees: <strong>${grossTaxRushFees.toLocaleString()}</strong>
                    </div>
                    <div style={{ color: '#0369a1', fontSize: '0.8rem' }}>
                      Returns: {currentTaxRushReturns.toLocaleString()} @ $
                      {currentTaxRushAvgNetFee.toLocaleString()}
                    </div>
                    <div style={{ color: '#dc2626', fontSize: '0.8rem' }}>
                      Less Discounts ({discountPct}%):{' '}
                      <strong>-${Math.round(taxRushDiscountAmount).toLocaleString()}</strong>
                    </div>
                    <div style={{ fontWeight: 'bold', color: '#059669', fontSize: '0.85rem' }}>
                      Net TaxRush Income:{' '}
                      <strong>${Math.round(taxRushIncome).toLocaleString()}</strong>
                    </div>
                  </div>
                )}
              </div>
              {answers.hasOtherIncome && answers.otherIncome && (
                <div style={{ color: statusColor }}>
                  Other Revenue: <strong>${answers.otherIncome.toLocaleString()}</strong>
                </div>
              )}
              ...
              >
                ...
                {strategicTarget > 0 && (
                  <span style={{ fontSize: '0.85rem', marginLeft: '0.5rem' }}>
                    ({revenueVariance > 0 ? '+' : ''}
                    {Math.round(revenueVariance)}% vs target)
                  </span>
                )}
              </div>

              <div style={{ fontSize: '0.75rem', color: statusColor, marginTop: '0.5rem' }}>
                This is your projected gross revenue before expenses. Your net income (profit) will
                be this amount minus all expenses below.
              </div>
            </div>
          );
        })()}
      ...
      >
        ...
        {(() => {
          ...
          const currentTaxRushReturns =
            ...
            answers.taxRushReturns &&
            answers.expectedGrowthPct !== undefined
              ? answers.taxRushReturns * (1 + answers.expectedGrowthPct / 100)
              : answers.handlesTaxRush
                ? answers.taxRushReturns || 0
                : 0;
          const currentTaxRushAvgNetFee =
            answers.handlesTaxRush &&
            answers.taxRushAvgNetFee &&
            answers.expectedGrowthPct !== undefined
              ? answers.taxRushAvgNetFee * (1 + answers.expectedGrowthPct / 100)
              : answers.handlesTaxRush
                ? answers.taxRushAvgNetFee || 0
                : 0;
          ...
          if (currentAvgNetFee && currentTaxPrepReturns) {
            ...
            const taxRushIncome =
              answers.handlesTaxRush && currentTaxRushAvgNetFee && currentTaxRushReturns
                ? currentTaxRushAvgNetFee * currentTaxRushReturns
                : 0;
            ...
            const expectedRevenueMatchesActual =
            ...
            const scalingFactor =
              rawExpenseTotal > 0 ? targetExpensePercentage / rawExpenseTotal : 1;
            ...
            return (
              ...
              >
                ...
                <div style={{ color: '#1e40af' }}>
                  Strategic Total Expenses:{' '}
                  <strong>${Math.round(strategicExpenseTarget).toLocaleString()}</strong>
                  <span style={{ fontSize: '0.85rem', marginLeft: '0.5rem' }}>
                ...
                <div style={{ color: '#1e40af', marginTop: '0.25rem' }}>
                  Expected Net Income:{' '}
                  <strong>${Math.round(strategicNetIncome).toLocaleString()}</strong>
                  <span style={{ fontSize: '0.85rem', marginLeft: '0.5rem' }}>
                ...
                {!expectedRevenueMatchesActual &&
                  ...
                  Math.abs(answers.expectedRevenue - totalGrossRevenue) > 5000 && (
                    ...
                    >
                      <div
                        style={{ fontWeight: 'bold', color: '#92400e', marginBottom: '0.25rem' }}
                      >
                        ⚠️ Revenue Calculation Notice
                      </div>
                      <div style={{ color: '#92400e', lineHeight: 1.4 }}>
                        There's a difference between your Page 1 projections ($
                        {answers.expectedRevenue?.toLocaleString()}) and Page 2 calculations ($
                        {Math.round(totalGrossRevenue).toLocaleString()}). This might indicate
                        different growth rates or base amounts between pages.
                      </div>
                    </div>
                  )}
        ...
        {(() => {
          ...
          if (currentAvgNetFee && currentTaxPrepReturns) {
            ...
            const targetExpensePercentage = 76;
            const scalingFactor =
              rawExpenseForScaling > 0 ? targetExpensePercentage / rawExpenseForScaling : 1;

            // Calculate total actual expenses from all expense fields
            // Scale user's defaults to match strategic targets for better UX
            const userHasMadeExpenseChanges = relevantFields.some(
              (field) =>
                (answers as any)[field.id] !== undefined &&
                (answers as any)[field.id] !== field.defaultValue
            );
            ...
            const actualTotalExpenses = relevantFields.reduce((total, field) => {
              ...
              if (field.calculationBase === 'fixed_amount') {
                ...
                const salariesAmount =
                  (totalGrossRevenue * ((answers as any).salariesPct || 25)) / 100;
            ...
            React.useEffect(() => {
              if (
                ...
                Math.abs(actualTotalExpenses - (answers.calculatedTotalExpenses || 0)) > 1
              ) {
            ...
            const actualExpensePercentage =
              totalGrossRevenue > 0 ? (actualTotalExpenses / totalGrossRevenue) * 100 : 0;
            ...
            const expenseVariance =
              strategicExpenseTarget > 0
                ? ((actualTotalExpenses - strategicExpenseTarget) / strategicExpenseTarget) * 100
                : 0;
            ...
            if (actualExpensePercentage >= 74.5 && actualExpensePercentage <= 77.5) {
              ...
              (actualExpensePercentage >= 71.5 && actualExpensePercentage < 74.5) ||
              ...
              statusText =
                actualExpensePercentage < 74.5
                  ? 'Below optimal range - monitor operations'
                  : 'Above optimal range - monitor costs';
            } else {
              ...
              statusText =
                actualExpensePercentage < 71.5
                  ? 'Under-investment risk - ensure quality standards'
                  : 'Over budget - reduce expenses immediately';
            }
            ...
            return (
              ...
              >
                ...
                <div style={{ color: statusColor }}>
                  Total Expenses:{' '}
                  <strong>${Math.round(actualTotalExpenses).toLocaleString()}</strong>
                  <span style={{ fontSize: '0.85rem', marginLeft: '0.5rem' }}>
                ...
                <div style={{ color: statusColor, marginTop: '0.25rem' }}>
                  ...
                  <span
                    style={{
                      ...
                      marginLeft: '0.5rem',
                      color:
                        actualExpensePercentage < 71.5 || actualExpensePercentage > 80.5
                          ? '#dc2626'
                          : statusColor,
                      fontWeight: 'bold',
                    }}
                  >
                    {actualExpensePercentage > 80.5
                      ? '⚠️ Over optimal range'
                      : actualExpensePercentage < 71.5
                        ? '⚠️ Under-investment risk'
                        : '✅ Within operational best practices'}
                  </span>
                </div>

                <div className="small" style={{ color: statusColor, marginTop: '0.25rem' }}>
                  Operational Target: ${Math.round(strategicExpenseTarget).toLocaleString()} (
                  {((strategicExpenseTarget / totalGrossRevenue) * 100).toFixed(1)}% of revenue) •
                  {actualExpensePercentage > 80.5
