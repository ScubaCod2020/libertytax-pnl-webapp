...
export default function SuggestionManager({ region, onProfileUpdate }: SuggestionManagerProps) {
  ...
  const relevantProfiles = Object.entries(suggestionProfiles).filter(
    ([key, profile]) => profile.region === region
  );
    ...
    >
      ...
      {currentProfile && (
        <>
          ...
          >
            <h4 style={{ margin: '0 0 0.5rem 0', color: '#f3f4f6' }}>{currentProfile.name}</h4>
          ...
          {suggestions && (
            ...
            >
              ...
              <div style={{ fontSize: '0.8rem', lineHeight: 1.4 }}>
                <div>
                  ANF ${suggestions.avgNetFee} × {suggestions.taxPrepReturns} returns ={' '}
                  <strong>${suggestions.grossFees.toLocaleString()}</strong> gross
                </div>
                <div>
                  Less {suggestions.discountsPct}% discounts ={' '}
                  <strong>${suggestions.taxPrepIncome.toLocaleString()}</strong> tax prep income
                </div>
                {suggestions.taxRushIncome > 0 && (
                  <div>
                    Plus TaxRush ${suggestions.taxRushIncome.toLocaleString()} ={' '}
                    <strong>${suggestions.totalRevenue.toLocaleString()}</strong> total
                  </div>
                )}
                <div>
                  Less ${suggestions.totalExpenses.toLocaleString()} expenses ={' '}
                  <strong>${suggestions.netIncome.toLocaleString()}</strong> net income
                </div>
              </div>
            </div>
          )}
                        ...
                        <>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateProfileField('taxRushReturns', +e.target.value)
                              }
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateProfileField('taxRushAvgNetFee', +e.target.value)
                              }
                      ...
                      {editedProfile.storeType === 'existing' && (
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateProfileField('expectedGrowthPct', +e.target.value)
                            }
                      ...
                      {[
                        ...
                        <div key={expenseId}>
                          ...
                          <input
                            ...
                            value={
                              editedProfile.expenses[
                                expenseId as keyof typeof editedProfile.expenses
                              ]
                            }
