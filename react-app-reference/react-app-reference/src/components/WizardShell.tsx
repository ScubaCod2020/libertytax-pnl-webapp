...
export default function WizardShell({
  ...
  const [visitedSteps, setVisitedSteps] = useState<Record<WizardStep, boolean>>({
    welcome: true,
    inputs: false,
    review: false,
  });
  ...
  const canProceed = () => {
    ...
    const hasLastYearBasics =
      (answers.lastYearTaxPrepReturns ?? 0) > 0 && (answers.manualAvgNetFee ?? 0) > 0;
  ...
  useEffect(() => {
    ...
    if (answers.storeType === 'new' && answers.avgNetFee && answers.taxPrepReturns) {
      ...
      const taxRushIncome =
        region === 'CA' && answers.taxRushReturns ? answers.avgNetFee * answers.taxRushReturns : 0;
    ...
    answers.lastYearDiscountsPct,
    answers.lastYearOtherIncome,
  ...
  if (currentStep === 'review') {
    return (
      <div style={{ paddingLeft: '1rem' }}>
        {stepNav}
        <WizardReview answers={answers} onNext={() => onComplete(answers)} onBack={handleBack} />
      </div>
    );
  }
...
function WelcomeStep({
  ...
  return (
    <div data-wizard-step="welcome" style={{ paddingLeft: '1rem' }}>
      ...
      {/* Region Selection */}
      <FormField label="Region" helpText="Affects tax calculations and available features" required>
        <label htmlFor="region-select" style={{ fontWeight: 500, display: 'none' }}>
          Region
        </label>
        <select
          id="region-select"
      ...
      <FormField
        ...
        required
      >
        <label htmlFor="store-type-select" style={{ fontWeight: 500, display: 'none' }}>
          Store Type
        </label>
        <select
      ...
      {answers.storeType === 'existing' && (
        <ExistingStoreSection answers={answers} updateAnswers={updateAnswers} region={region} />
      )}

      {answers.storeType === 'new' && (
        <NewStoreSection answers={answers} updateAnswers={updateAnswers} region={region} />
      )}
      ...
      {answers.storeType === 'existing' && (
        <StrategicAnalysis answers={answers} updateAnswers={updateAnswers} region={region} />
      )}
      ...
      {answers.projectedExpenses && answers.expectedRevenue && (
        ...
        >
          ...
          {Math.round(answers.expectedRevenue - answers.projectedExpenses).toLocaleString()}
          <div style={{ fontSize: '0.8rem', fontWeight: 'normal' }}>
            Net Margin:{' '}
            {Math.round(
              ((answers.expectedRevenue - answers.projectedExpenses) / answers.expectedRevenue) *
                100
            )}
            %
          </div>
