...
import { getAdjustmentStatus, calculateBlendedGrowth, calculatePerformanceVsTarget } from './calculations'
...
export default function StrategicAnalysis({ answers }: WizardSectionProps) {
  ...
  if (adjustments.avgNetFeeStatus) {
    const growth = answers.projectedAvgNetFee && answers.avgNetFee 
  ...
  if (adjustments.taxPrepReturnsStatus) {
    const growth = answers.projectedTaxPrepReturns && answers.taxPrepReturns 
      ? Math.round(((answers.projectedTaxPrepReturns - answers.taxPrepReturns) / answers.taxPrepReturns) * 100)
      : 0
    const variance = growth - originalGrowth
    adjustmentsList.push({
