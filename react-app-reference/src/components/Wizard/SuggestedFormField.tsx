...
export default function SuggestedFormField({ 
  ...
  const formatSuggestion = (value: number): string => {
    ...
    // Currency fields
    if (fieldId.includes('fees') || fieldId.includes('income') || fieldId.includes('revenue') || 
        fieldId.includes('expenses') || fieldId.includes('Amount')) {
...
export function SuggestedCurrencyInput({ 
  ...
  const displayPlaceholder = placeholder || (suggestedValue ? `${suggestedValue.toLocaleString()}` : undefined)
...
export function SuggestedPercentageInput({ 
  ...
  const suggestedValue = suggestions?.[fieldId] as number
  const displayPlaceholder = placeholder || (suggestedValue ? `${suggestedValue.toFixed(1)}` : undefined)
