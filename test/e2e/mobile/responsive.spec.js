...
test.describe('Mobile Responsiveness', () => {
  ...
  test('region selector is accessible on mobile', async ({ page }) => {
    const regionRadios = page.locator('input[aria-label="Region"]');
    await expect(regionRadios.first()).toBeVisible();
    const box = await regionRadios.first().boundingBox();
...
test.describe('Touch Interactions', () => {
  ...
  test('select region works with touch (radios)', async ({ page }) => {
    const regionCA = page.locator('input[aria-label="Region"][value="CA"]');
    await regionCA.tap();
    await expect(regionCA).toBeChecked();
  });
