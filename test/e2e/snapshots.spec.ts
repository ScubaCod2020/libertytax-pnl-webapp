...
test.describe('UI Snapshots', () => {
  test('capture key screens', async ({ page, baseURL }) => {
    const dateTag = process.env.SNAP_DATE || new Date().toISOString().slice(0,10).replace(/-/g, '')
    const snapBase = process.env.SNAP_DIR || path.join(process.cwd(), 'docs', 'ui-snapshots', dateTag)
