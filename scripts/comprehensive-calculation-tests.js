...
function runComprehensiveTests() {
  ...
  Object.entries(testScenarios).forEach(([key, scenario]) => {
    ...
    try {
      ...
      const expectedTotalReturns =
        scenario.taxPrepReturns + (scenario.region === 'CA' ? scenario.taxRushReturns : 0);
      ...
      checks.forEach((check) => {
        ...
        console.log(
          `  ${passed ? '✅' : '❌'} ${check.name}: Expected $${check.expected.toLocaleString()}, Got $${check.actual.toLocaleString()}`
        );
        if (!passed) {
  ...
  console.log(
    `🇨🇦 CA Region: ${caPassed}/${caTotal} passed (${((caPassed / caTotal) * 100).toFixed(1)}%)`
  );
