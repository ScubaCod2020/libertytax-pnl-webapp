...
function wizardCalculateBase(answers, calculationBase) {
  ...
  const grossFees =
    answers.avgNetFee && answers.taxPrepReturns ? answers.avgNetFee * answers.taxPrepReturns : 0;
  ...
  switch (calculationBase) {
    case 'percentage_gross':
      ...
      return answers.expectedRevenue
        ? answers.expectedRevenue / (1 - (answers.discountsPct || 3) / 100)
        : grossFees;
    ...
    case 'percentage_salaries':
      const baseGrossFees = answers.expectedRevenue
        ? answers.expectedRevenue / (1 - (answers.discountsPct || 3) / 100)
        : grossFees;
...
testCases.forEach((testCase, index) => {
  ...
  const wizardTaxRushRoyalties =
    testCase.region === 'CA' ? wizardTpIncomeBase * (testCase.taxRushRoyaltiesPct / 100) : 0;

  console.log(
    `Input: ANF $${testCase.avgNetFee}, Returns ${testCase.taxPrepReturns}, Discounts ${testCase.discountsPct}%`
  );
  ...
  console.log(
    `  Salaries (${testCase.salariesPct}% of Gross): $${mainResult.salaries.toLocaleString()}`
  );
  console.log(
    `  Royalties (${testCase.royaltiesPct}% of TP Income): $${mainResult.royalties.toLocaleString()}`
  );
  if (testCase.region === 'CA') {
    console.log(
      `  TaxRush Royalties (${testCase.taxRushRoyaltiesPct}% of TP Income): $${mainResult.taxRushRoyalties.toLocaleString()}`
    );
  }
